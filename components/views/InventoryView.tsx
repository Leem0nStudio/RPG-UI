...
export function InventoryView({ targetSlot, inventory, onBack, onEquip, onUnequip, equippedItem }: { targetSlot: ItemType | null, inventory: Item[], onBack: () => void, onEquip: (i:Item)=>void, onUnequip: (slot:ItemType)=>void, equippedItem: Item | null }) {
  ...
  return (
    <div className="w-full flex-1 flex flex-col pt-1 pb-4 animate-in slide-in-from-right-4 duration-200 ui-text">
      
      {/* Header */}
      <div className="flex items-center justify-between bg-gradient-to-r from-[#4a2e1a] to-[#2a1708] border-[2px] border-[var(--color-accent-gold)] rounded-[4px] p-2 mb-3 sm:mb-4 shadow-[var(--shadow-medium)]">
        ...
        <div className="flex-1 text-center mr-2 sm:mr-6">
           <span className="ui-heading font-bold text-[#f2e6d5] text-[14px] sm:text-[16px] text-stroke-sm tracking-wide sm:tracking-widest uppercase">
             SELECT {targetSlot}
           </span>
        </div>
      </div>
      ...
      <div className="bg-[#2c1d11] border-[2px] border-[#1a110a] rounded-[4px] p-2 mb-4 relative shadow-[inset_0_0_10px_rgba(0,0,0,0.7)]">
           ...
           <div className="flex flex-col sm:flex-row items-start sm:items-center gap-2 sm:gap-3 mt-2">
              ...
              <button 
                onClick={() => targetSlot && onUnequip(targetSlot)}
                 className="bg-gradient-to-b from-[#5c5c5c] to-[#222222] border-[1.5px] border-[#888] rounded px-2 py-1 text-[11px] font-bold text-white text-stroke-black hover:brightness-110 active:scale-95 transition-all duration-200 whitespace-nowrap shadow-md w-full sm:w-auto"
              >
                Unequip
              </button>
           </div>
      ...
      <div className="flex-1 overflow-y-auto space-y-2 pr-1">
        {filteredItems.map((item, index) => {
           ...
           return (
              <div key={item.id} className="group relative ui-panel rounded-[4px] p-2 flex flex-col sm:flex-row items-start sm:items-center gap-2 sm:gap-3 rpg-panel-shadow hover:z-50 cursor-help">
                 ...
                 <div className="flex flex-col flex-1 leading-tight py-1 min-w-0 w-full">
                 ...
                 <button 
                  ...
                  disabled={isEquipped}
                  className={`flex-shrink-0 relative z-20 px-3 py-1.5 rounded border-[1.5px] text-[11px] font-bold text-stroke-black shadow-[0_2px_4px_rgba(0,0,0,0.5)] transition-all duration-200 w-full sm:w-auto ${isEquipped ? 'bg-[#555] border-[#333] text-gray-300 opacity-80 cursor-default' : 'bg-gradient-to-b from-[#6bb84c] to-[#3a7522] border-[#9ceb7a] text-white hover:brightness-125 active:scale-95 cursor-pointer'}`}
                 >
