import type { GameBootstrap, GameContent, PlayerProfile, OwnedUnit, SummonBanner, QuestDefinition, JobDefinition } from '@/backend-contracts/game';

const jobs: JobDefinition[] = [
  // === 1st Class (Tier 1) ===
  {
    id: 'job_swordman',
    name: 'Swordman',
    tier: '1',
    category: 'Sword',
    spriteUrl: '/assets/sprites/characters/1job/swordman_.png',
    cssFilter: '',
    requiredJobLevel: 10,
    evolvedFrom: null,
    baseStats: { hp: 200, atk: 20, def: 15, rec: 5 },
    statMultipliers: { hp: 1.2, atk: 1.1, def: 1.0, rec: 0.8 },
    skills: [],
  },
  {
    id: 'job_archer',
    name: 'Archer',
    tier: '1',
    category: 'Bow',
    spriteUrl: '/assets/sprites/characters/1job/archer_.png',
    cssFilter: '',
    requiredJobLevel: 10,
    evolvedFrom: null,
    baseStats: { hp: 160, atk: 28, def: 8, rec: 8 },
    statMultipliers: { hp: 0.9, atk: 1.3, def: 0.6, rec: 0.9 },
    skills: [],
  },
  {
    id: 'job_mage',
    name: 'Mage',
    tier: '1',
    category: 'Magic',
    spriteUrl: '/assets/sprites/characters/1job/mage_.png',
    cssFilter: '',
    requiredJobLevel: 10,
    evolvedFrom: null,
    baseStats: { hp: 120, atk: 30, def: 5, rec: 25 },
    statMultipliers: { hp: 0.8, atk: 1.4, def: 0.5, rec: 1.3 },
    skills: [],
  },
  {
    id: 'job_thief',
    name: 'Thief',
    tier: '1',
    category: 'Thief',
    spriteUrl: '/assets/sprites/characters/1job/thief_.png',
    cssFilter: '',
    requiredJobLevel: 10,
    evolvedFrom: null,
    baseStats: { hp: 150, atk: 25, def: 8, rec: 10 },
    statMultipliers: { hp: 0.9, atk: 1.3, def: 0.7, rec: 1.0 },
    skills: [],
  },
  {
    id: 'job_merchant',
    name: 'Merchant',
    tier: '1',
    category: 'Trade',
    spriteUrl: '/assets/sprites/characters/1job/merchant_.png',
    cssFilter: '',
    requiredJobLevel: 10,
    evolvedFrom: null,
    baseStats: { hp: 180, atk: 15, def: 12, rec: 12 },
    statMultipliers: { hp: 1.1, atk: 0.9, def: 1.0, rec: 1.0 },
    skills: [],
  },
  {
    id: 'job_acolyte',
    name: 'Acolyte',
    tier: '1',
    category: 'Heal',
    spriteUrl: '/assets/sprites/characters/1job/acolyte_.png',
    cssFilter: '',
    requiredJobLevel: 10,
    evolvedFrom: null,
    baseStats: { hp: 140, atk: 10, def: 10, rec: 30 },
    statMultipliers: { hp: 1.0, atk: 0.7, def: 0.8, rec: 1.5 },
    skills: [],
  },

  // === 2nd Class (Tier 2) ===
  {
    id: 'job_knight',
    name: 'Knight',
    tier: '2',
    category: 'Sword',
    spriteUrl: '/assets/sprites/characters/2job/knight_.png',
    cssFilter: '',
    requiredJobLevel: 10,
    evolvedFrom: 'job_swordman',
    baseStats: { hp: 350, atk: 35, def: 25, rec: 10 },
    statMultipliers: { hp: 1.4, atk: 1.3, def: 1.2, rec: 0.8 },
    skills: [],
  },
  {
    id: 'job_crusader',
    name: 'Crusader',
    tier: '2',
    category: 'Sword',
    spriteUrl: '/assets/sprites/characters/1job/swordman_.png',
    cssFilter: 'sepia(0.3) brightness(0.9)',
    requiredJobLevel: 10,
    evolvedFrom: 'job_swordman',
    baseStats: { hp: 380, atk: 28, def: 30, rec: 15 },
    statMultipliers: { hp: 1.5, atk: 1.1, def: 1.4, rec: 0.9 },
    skills: [],
  },
  {
    id: 'job_hunter',
    name: 'Hunter',
    tier: '2',
    category: 'Bow',
    spriteUrl: '/assets/sprites/characters/1job/archer_.png',
    cssFilter: 'saturate(1.2)',
    requiredJobLevel: 10,
    evolvedFrom: 'job_archer',
    baseStats: { hp: 280, atk: 45, def: 12, rec: 15 },
    statMultipliers: { hp: 1.0, atk: 1.5, def: 0.7, rec: 1.0 },
    skills: [],
  },
  {
    id: 'job_bard',
    name: 'Bard',
    tier: '2',
    category: 'Bow',
    spriteUrl: '/assets/sprites/characters/1job/archer_.png',
    cssFilter: 'hue-rotate(180deg) saturate(0.8)',
    requiredJobLevel: 10,
    evolvedFrom: 'job_archer',
    baseStats: { hp: 220, atk: 35, def: 10, rec: 25 },
    statMultipliers: { hp: 0.9, atk: 1.3, def: 0.6, rec: 1.3 },
    skills: [],
  },
  {
    id: 'job_wizard',
    name: 'Wizard',
    tier: '2',
    category: 'Magic',
    spriteUrl: '/assets/sprites/characters/2job/wizard_.png',
    cssFilter: '',
    requiredJobLevel: 10,
    evolvedFrom: 'job_mage',
    baseStats: { hp: 200, atk: 55, def: 8, rec: 40 },
    statMultipliers: { hp: 0.8, atk: 1.8, def: 0.5, rec: 1.5 },
    skills: [],
  },
  {
    id: 'job_sage',
    name: 'Sage',
    tier: '2',
    category: 'Magic',
    spriteUrl: '/assets/sprites/characters/2job/wizard_.png',
    cssFilter: 'hue-rotate(30deg) brightness(1.1)',
    requiredJobLevel: 10,
    evolvedFrom: 'job_mage',
    baseStats: { hp: 190, atk: 45, def: 12, rec: 50 },
    statMultipliers: { hp: 0.85, atk: 1.5, def: 0.7, rec: 1.7 },
    skills: [],
  },
  {
    id: 'job_assassin',
    name: 'Assassin',
    tier: '2',
    category: 'Thief',
    spriteUrl: '/assets/sprites/characters/2job/assasin_.png',
    cssFilter: '',
    requiredJobLevel: 10,
    evolvedFrom: 'job_thief',
    baseStats: { hp: 250, atk: 45, def: 12, rec: 18 },
    statMultipliers: { hp: 1.0, atk: 1.6, def: 0.7, rec: 1.1 },
    skills: [],
  },
  {
    id: 'job_rogue',
    name: 'Rogue',
    tier: '2',
    category: 'Thief',
    spriteUrl: '/assets/sprites/characters/1job/thief_.png',
    cssFilter: 'brightness(0.8) saturate(1.3)',
    requiredJobLevel: 10,
    evolvedFrom: 'job_thief',
    baseStats: { hp: 230, atk: 40, def: 15, rec: 20 },
    statMultipliers: { hp: 0.95, atk: 1.5, def: 0.8, rec: 1.2 },
    skills: [],
  },
  {
    id: 'job_blacksmith',
    name: 'Blacksmith',
    tier: '2',
    category: 'Trade',
    spriteUrl: '/assets/sprites/characters/2job/blacksmith_.png',
    cssFilter: '',
    requiredJobLevel: 10,
    evolvedFrom: 'job_merchant',
    baseStats: { hp: 300, atk: 25, def: 20, rec: 20 },
    statMultipliers: { hp: 1.3, atk: 1.1, def: 1.2, rec: 1.1 },
    skills: [],
  },
  {
    id: 'job_alchemist',
    name: 'Alchemist',
    tier: '2',
    category: 'Trade',
    spriteUrl: '/assets/sprites/characters/2job/blacksmith_.png',
    cssFilter: 'hue-rotate(90deg) saturate(0.7)',
    requiredJobLevel: 10,
    evolvedFrom: 'job_merchant',
    baseStats: { hp: 280, atk: 20, def: 18, rec: 25 },
    statMultipliers: { hp: 1.25, atk: 0.9, def: 1.1, rec: 1.3 },
    skills: [],
  },
  {
    id: 'job_priest',
    name: 'Priest',
    tier: '2',
    category: 'Heal',
    spriteUrl: '/assets/sprites/characters/2job/priest_.png',
    cssFilter: '',
    requiredJobLevel: 10,
    evolvedFrom: 'job_acolyte',
    baseStats: { hp: 230, atk: 15, def: 15, rec: 50 },
    statMultipliers: { hp: 1.1, atk: 0.7, def: 0.9, rec: 1.8 },
    skills: [],
  },
  {
    id: 'job_monk',
    name: 'Monk',
    tier: '2',
    category: 'Heal',
    spriteUrl: '/assets/sprites/characters/2job/priest_.png',
    cssFilter: 'sepia(0.2) saturate(1.2)',
    requiredJobLevel: 10,
    evolvedFrom: 'job_acolyte',
    baseStats: { hp: 280, atk: 30, def: 15, rec: 30 },
    statMultipliers: { hp: 1.2, atk: 1.2, def: 0.9, rec: 1.2 },
    skills: [],
  },

  // === Transcendent (Tier 3) ===
  {
    id: 'job_lord_knight',
    name: 'Lord Knight',
    tier: '3',
    category: 'Sword',
    spriteUrl: '/assets/sprites/characters/2job/knight_.png',
    cssFilter: 'brightness(1.2) saturate(1.1)',
    requiredJobLevel: 10,
    evolvedFrom: 'job_knight',
    baseStats: { hp: 450, atk: 50, def: 35, rec: 15 },
    statMultipliers: { hp: 1.6, atk: 1.5, def: 1.4, rec: 0.9 },
    skills: [],
  },
  {
    id: 'job_paladin',
    name: 'Paladin',
    tier: '3',
    category: 'Sword',
    spriteUrl: '/assets/sprites/characters/2job/knight_.png',
    cssFilter: 'sepia(0.4) brightness(1.1)',
    requiredJobLevel: 10,
    evolvedFrom: 'job_crusader',
    baseStats: { hp: 480, atk: 40, def: 42, rec: 20 },
    statMultipliers: { hp: 1.7, atk: 1.3, def: 1.6, rec: 1.0 },
    skills: [],
  },
  {
    id: 'job_sniper',
    name: 'Sniper',
    tier: '3',
    category: 'Bow',
    spriteUrl: '/assets/sprites/characters/1job/archer_.png',
    cssFilter: 'saturate(1.5) brightness(1.1)',
    requiredJobLevel: 10,
    evolvedFrom: 'job_hunter',
    baseStats: { hp: 350, atk: 60, def: 18, rec: 20 },
    statMultipliers: { hp: 1.1, atk: 1.7, def: 0.8, rec: 1.1 },
    skills: [],
  },
  {
    id: 'job_clown',
    name: 'Clown',
    tier: '3',
    category: 'Bow',
    spriteUrl: '/assets/sprites/characters/1job/archer_.png',
    cssFilter: 'hue-rotate(200deg) saturate(1.2)',
    requiredJobLevel: 10,
    evolvedFrom: 'job_bard',
    baseStats: { hp: 280, atk: 48, def: 15, rec: 35 },
    statMultipliers: { hp: 1.0, atk: 1.5, def: 0.7, rec: 1.5 },
    skills: [],
  },
  {
    id: 'job_high_wizard',
    name: 'High Wizard',
    tier: '3',
    category: 'Magic',
    spriteUrl: '/assets/sprites/characters/2job/wizard_.png',
    cssFilter: 'brightness(1.2)',
    requiredJobLevel: 10,
    evolvedFrom: 'job_wizard',
    baseStats: { hp: 260, atk: 75, def: 12, rec: 55 },
    statMultipliers: { hp: 0.9, atk: 2.0, def: 0.6, rec: 1.7 },
    skills: [],
  },
  {
    id: 'job_professor',
    name: 'Professor',
    tier: '3',
    category: 'Magic',
    spriteUrl: '/assets/sprites/characters/2job/wizard_.png',
    cssFilter: 'hue-rotate(40deg) brightness(1.05)',
    requiredJobLevel: 10,
    evolvedFrom: 'job_sage',
    baseStats: { hp: 250, atk: 60, def: 18, rec: 65 },
    statMultipliers: { hp: 0.95, atk: 1.7, def: 0.8, rec: 1.9 },
    skills: [],
  },
  {
    id: 'job_assassin_cross',
    name: 'Assassin Cross',
    tier: '3',
    category: 'Thief',
    spriteUrl: '/assets/sprites/characters/2job/assasin_.png',
    cssFilter: 'brightness(1.1) saturate(1.2)',
    requiredJobLevel: 10,
    evolvedFrom: 'job_assassin',
    baseStats: { hp: 320, atk: 60, def: 18, rec: 25 },
    statMultipliers: { hp: 1.1, atk: 1.8, def: 0.8, rec: 1.2 },
    skills: [],
  },
  {
    id: 'job_stalker',
    name: 'Stalker',
    tier: '3',
    category: 'Thief',
    spriteUrl: '/assets/sprites/characters/1job/thief_.png',
    cssFilter: 'brightness(0.7) saturate(1.5)',
    requiredJobLevel: 10,
    evolvedFrom: 'job_rogue',
    baseStats: { hp: 300, atk: 55, def: 20, rec: 28 },
    statMultipliers: { hp: 1.05, atk: 1.7, def: 0.9, rec: 1.3 },
    skills: [],
  },
  {
    id: 'job_whitesmith',
    name: 'Whitesmith',
    tier: '3',
    category: 'Trade',
    spriteUrl: '/assets/sprites/characters/2job/blacksmith_.png',
    cssFilter: 'brightness(1.15)',
    requiredJobLevel: 10,
    evolvedFrom: 'job_blacksmith',
    baseStats: { hp: 380, atk: 35, def: 28, rec: 28 },
    statMultipliers: { hp: 1.5, atk: 1.3, def: 1.4, rec: 1.2 },
    skills: [],
  },
  {
    id: 'job_creator',
    name: 'Creator',
    tier: '3',
    category: 'Trade',
    spriteUrl: '/assets/sprites/characters/2job/blacksmith_.png',
    cssFilter: 'hue-rotate(100deg) saturate(0.8)',
    requiredJobLevel: 10,
    evolvedFrom: 'job_alchemist',
    baseStats: { hp: 360, atk: 28, def: 25, rec: 35 },
    statMultipliers: { hp: 1.4, atk: 1.0, def: 1.3, rec: 1.5 },
    skills: [],
  },
  {
    id: 'job_high_priest',
    name: 'High Priest',
    tier: '3',
    category: 'Heal',
    spriteUrl: '/assets/sprites/characters/2job/priest_.png',
    cssFilter: 'brightness(1.15)',
    requiredJobLevel: 10,
    evolvedFrom: 'job_priest',
    baseStats: { hp: 300, atk: 22, def: 22, rec: 65 },
    statMultipliers: { hp: 1.2, atk: 0.8, def: 1.0, rec: 2.0 },
    skills: [],
  },
  {
    id: 'job_champion',
    name: 'Champion',
    tier: '3',
    category: 'Heal',
    spriteUrl: '/assets/sprites/characters/2job/priest_.png',
    cssFilter: 'sepia(0.3) saturate(1.3)',
    requiredJobLevel: 10,
    evolvedFrom: 'job_monk',
    baseStats: { hp: 360, atk: 42, def: 22, rec: 42 },
    statMultipliers: { hp: 1.35, atk: 1.4, def: 1.0, rec: 1.4 },
    skills: [],
  },
];

const units: GameContent['units'] = [
  {
    id: 'u_sergio',
    name: 'Knight Sergio',
    title: 'ROGUE TIDE',
    element: 'Water',
    rarity: 3,
    maxLevel: 40,
    jobId: 'job_swordman',
    maxJobLevel: 50,
    cost: 5,
    baseStats: { hp: 2050, atk: 600, def: 600, rec: 580 },
    skills: [
      { id: 'ls_sergio', type: 'LEADER SKILL', title: 'Tidal Command', description: '10% boost to Atk Power of Water units', iconType: 'Flag' },
      { id: 'bb_sergio', type: 'BRAVE BURST', title: 'Aqua Slash', description: '5 combo Water attack on a single enemy', cost: 18, iconType: 'Sparkles' },
    ],
  },
  {
    id: 'u_vargas',
    name: 'Vargas',
    title: 'EMBER VANGUARD',
    element: 'Fire',
    rarity: 4,
    maxLevel: 40,
    jobId: 'job_swordman',
    maxJobLevel: 50,
    cost: 8,
    baseStats: { hp: 2400, atk: 850, def: 550, rec: 400 },
    skills: [
      { id: 'ls_vargas', type: 'LEADER SKILL', title: 'Flame March', description: '15% boost to Atk Power of Fire units', iconType: 'Flag' },
      { id: 'bb_vargas', type: 'BRAVE BURST', title: 'Flare Ride', description: '8 combo Fire attack on all enemies', cost: 22, iconType: 'Sparkles' },
    ],
  },
  {
    id: 'u_lance',
    name: 'Lance',
    title: 'STONE PIKEMAN',
    element: 'Earth',
    rarity: 3,
    maxLevel: 40,
    jobId: 'job_thief',
    maxJobLevel: 50,
    cost: 6,
    baseStats: { hp: 3100, atk: 550, def: 800, rec: 350 },
    skills: [
      { id: 'ls_lance', type: 'LEADER SKILL', title: 'Earthen Wall', description: '10% boost to HP of Earth units', iconType: 'Flag' },
      { id: 'bb_lance', type: 'BRAVE BURST', title: 'Tremor Strike', description: '6 combo Earth attack on all enemies', cost: 20, iconType: 'Sparkles' },
    ],
  },
  {
    id: 'u_magress',
    name: 'Magress',
    title: 'SHADOW AEGIS',
    element: 'Dark',
    rarity: 5,
    maxLevel: 60,
    jobId: 'job_knight',
    maxJobLevel: 50,
    cost: 12,
    baseStats: { hp: 4200, atk: 900, def: 950, rec: 200 },
    skills: [
      { id: 'ls_magress', type: 'LEADER SKILL', title: 'Dark Aegis', description: '15% boost to Def of Dark units', iconType: 'Flag' },
      { id: 'bb_magress', type: 'BRAVE BURST', title: 'Void Cleave', description: '7 combo Dark attack on all enemies', cost: 24, iconType: 'Sparkles' },
      { id: 'ex_magress', type: 'EXTRA SKILL', title: 'Shadow Veil', description: '20% boost to all parameters with the right Sphere', iconType: 'Sword' },
    ],
  },
  {
    id: 'u_elys',
    name: 'Elys',
    title: 'SOLAR SAINT',
    element: 'Light',
    rarity: 6,
    maxLevel: 80,
    jobId: 'job_knight',
    maxJobLevel: 50,
    cost: 15,
    baseStats: { hp: 4500, atk: 1200, def: 800, rec: 600 },
    skills: [
      { id: 'ls_elys', type: 'LEADER SKILL', title: 'Solar Blessing', description: '25% boost to Atk and 10% boost to HP of Light units', iconType: 'Flag' },
      { id: 'bb_elys', type: 'BRAVE BURST', title: 'Halo Breaker', description: '12 combo Light attack on all enemies', cost: 30, iconType: 'Sparkles' },
    ],
  },
];

const items: GameContent['items'] = [
  {
    id: 'w_brave_sword',
    name: 'Brave Sword',
    type: 'Weapon',
    rarity: 3,
    description: 'A standard sword assigned to brave warriors.',
    stats: { hp: 0, atk: 120, def: 0, rec: 0 },
    sprite: { col: 2, row: 0, className: 'drop-shadow-[0_2px_4px_#111]' },
    effects: ['10% chance to ignore enemy DEF'],
  },
  {
    id: 'w_flame_dagger',
    name: 'Flame Dagger',
    type: 'Weapon',
    rarity: 4,
    description: 'A dagger imbued with ember magic.',
    stats: { hp: 0, atk: 250, def: 0, rec: 50 },
    sprite: { col: 0, row: 2, className: 'drop-shadow-[0_2px_4px_#311]' },
    effects: ['Adds Fire element to attacks', '15% boost to Spark damage'],
  },
  {
    id: 'a_knight_shield',
    name: 'Knight Shield',
    type: 'Armor',
    rarity: 3,
    description: 'A sturdy iron shield.',
    stats: { hp: 200, atk: 0, def: 150, rec: 0 },
    sprite: { col: 3, row: 0, className: 'drop-shadow-[0_2px_4px_#111]' },
    effects: ['Reduces damage taken by 5%'],
  },
  {
    id: 'a_aura_plate',
    name: 'Aura Plate',
    type: 'Armor',
    rarity: 5,
    description: 'Legendary plate glowing with aura.',
    stats: { hp: 500, atk: 0, def: 400, rec: 100 },
    sprite: { col: 1, row: 1, className: 'drop-shadow-[0_2px_4px_#111]' },
    effects: ['Negates all status ailments', 'Restores HP each turn'],
  },
  {
    id: 'ac_hero_ring',
    name: 'Hero Ring',
    type: 'Accessory',
    rarity: 4,
    description: 'Boosts all stats slightly.',
    stats: { hp: 100, atk: 50, def: 50, rec: 50 },
    sprite: { col: 0, row: 1, className: 'drop-shadow-[0_2px_4px_#111]' },
  },
  {
    id: 'ac_recovery_amulet',
    name: 'Recovery Amulet',
    type: 'Accessory',
    rarity: 3,
    description: 'Enhances healing capability.',
    stats: { hp: 0, atk: 0, def: 0, rec: 300 },
    sprite: { col: 1, row: 0, className: 'drop-shadow-[0_2px_4px_#111]' },
    effects: ['Boosts BC and HC drop rate by 10%'],
  },
];

const quests: QuestDefinition[] = [
  {
    id: 'quest_evergreen_1',
    worldId: 'world_evergreen',
    stage: 1,
    name: 'Echoes of the Grove',
    energyCost: 5,
    difficulty: 'Normal',
    enemyIds: ['enemy_mimic', 'enemy_wisp', 'enemy_golem'],
    rewardsPreview: ['Zel x250', 'Karma x150', 'Potion', '1* Material'],
  },
  {
    id: 'quest_evergreen_2',
    worldId: 'world_evergreen',
    stage: 2,
    name: 'Sunlit Rift',
    energyCost: 7,
    difficulty: 'Normal',
    enemyIds: ['enemy_paladin', 'enemy_sprite', 'enemy_archer'],
    rewardsPreview: ['Zel x350', 'Karma x220', 'Fire Ore', 'Burst Crystal'],
  },
  {
    id: 'quest_abyss_1',
    worldId: 'world_abyss',
    stage: 1,
    name: 'Abyss Watch',
    energyCost: 9,
    difficulty: 'Hard',
    enemyIds: ['enemy_crawler', 'enemy_warden', 'enemy_magus'],
    rewardsPreview: ['Zel x500', 'Karma x300', 'Dark Relic', 'Evolution Totem'],
  },
];

const banners: SummonBanner[] = [
  {
    id: 'banner_origin_light',
    name: 'Origin of Light',
    cost: 5,
    currency: 'gems',
    featuredUnitIds: ['u_elys', 'u_magress'],
    description: 'Light and Dark featured rate-up banner with a guaranteed 4* or higher on multi.',
    active: true,
  },
];

export const defaultPlayerProfile: PlayerProfile = {
  id: 'player_local',
  name: 'Uptodown',
  level: 1,
  energy: {
    current: 18,
    max: 20,
    recoverAt: new Date(Date.now() + 12 * 60 * 1000).toISOString(),
  },
  currencies: {
    gems: 25,
    zel: 6420,
    karma: 1606,
  },
};

export const defaultRoster: OwnedUnit[] = [
  { instanceId: 'owned_sergio_1', unitId: 'u_sergio', level: 1, exp: 0, jobId: 'job_swordman', jobLevel: 1, jobExp: 0, locked: false, equipment: { Weapon: null, Armor: null, Accessory: null } },
  { instanceId: 'owned_vargas_1', unitId: 'u_vargas', level: 12, exp: 120, jobId: 'job_swordman', jobLevel: 5, jobExp: 50, locked: false, equipment: { Weapon: null, Armor: null, Accessory: null } },
  { instanceId: 'owned_lance_1', unitId: 'u_lance', level: 25, exp: 800, jobId: 'job_thief', jobLevel: 10, jobExp: 200, locked: false, equipment: { Weapon: null, Armor: null, Accessory: null } },
  { instanceId: 'owned_magress_1', unitId: 'u_magress', level: 30, exp: 4500, jobId: 'job_knight', jobLevel: 15, jobExp: 1000, locked: true, equipment: { Weapon: null, Armor: null, Accessory: null } },
];

export const defaultInventory = [
  { itemId: 'w_brave_sword', quantity: 1 },
  { itemId: 'w_flame_dagger', quantity: 1 },
  { itemId: 'a_knight_shield', quantity: 1 },
  { itemId: 'a_aura_plate', quantity: 1 },
  { itemId: 'ac_hero_ring', quantity: 1 },
  { itemId: 'ac_recovery_amulet', quantity: 2 },
];

export const gameContent: GameContent = {
  units,
  items,
  quests,
  banners,
  jobs,
};

export const bootstrapData: GameBootstrap = {
  player: defaultPlayerProfile,
  roster: defaultRoster,
  items: defaultInventory,
  content: gameContent,
};
